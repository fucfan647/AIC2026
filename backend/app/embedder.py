...
class Siglip2Embedder:
    def __init__(self, config: Siglip2Config):
        try:
            ...
            raise RuntimeError("torch and transformers are required for SigLIP-2 search") from exc
        ...
        print(f"[embedder] loading SigLIP-2 processor/tokenizer: {config.model_name}", flush=True)
        started = time.time()
        try:
            self.processor = AutoTokenizer.from_pretrained(
                config.model_name,
        ...
        print(f"[embedder] SigLIP-2 processor loaded in {time.time() - started:.2f}s", flush=True)

        print(f"[embedder] loading SigLIP-2 model: {config.model_name}", flush=True)
        started = time.time()
        self.model = AutoModel.from_pretrained(
            config.model_name,
            torch_dtype=torch_dtype,
        ...
        self.model.eval()
        print(f"[embedder] SigLIP-2 model loaded in {time.time() - started:.2f}s", flush=True)
    ...
    def encode_text_profiled(self, text: str):
        ...
        with self.torch.no_grad():
            ...
            if not isinstance(emb, self.torch.Tensor):
                if hasattr(emb, "pooler_output") and emb.pooler_output is not None:
                    ...
                    raise RuntimeError("SigLIP-2 model output does not expose text embeddings")
        self._sync()
        model_forward_ms = (time.perf_counter() - forward_started) * 1000.0
        ...
        arr = emb.detach().float().cpu().numpy().astype(np.float32)
        arr = l2_normalize(arr)
        if self.embedding_dim is not None and arr.shape != (1, self.embedding_dim):
            raise ValueError(f"SigLIP-2 text embedding shape must be (1, {self.embedding_dim}), got {arr.shape}")
        embedding_postprocess_ms = (time.perf_counter() - post_started) * 1000.0
