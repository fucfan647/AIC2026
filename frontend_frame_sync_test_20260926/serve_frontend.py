...
def create_app(
    ...
    def enrich_local_frame_timestamp(item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not re.fullmatch(r"[MNS]\d{2,3}[-_]V\d{3}", video_id):
            return item
        local_frames, _ = load_extended_local_metadata(video_id)
        keyframe_id = str(item.get("keyframe_id", "")).strip()
        match = next((frame for frame in local_frames if frame.get("keyframe_id") == keyframe_id), None)
        if match is None:
            normalized_keyframe = keyframe_id.replace("_", "-")
            match = next(
                (frame for frame in local_frames if str(frame.get("keyframe_id", "")).replace("_", "-") == normalized_keyframe),
                None,
            )
        if match is None:
            return item
    ...
    async def search(request: Request):
        try:
            ...
            for raw_item in payload.get("results", []):
                item = enrich_local_frame_timestamp(raw_item)
