...
# Check SigLIP-2 availability
SIGLIP2_DIR="${SIGLIP2_DIR:-$SCRIPT_DIR/siglip2_embeddings}"
SIGLIP2_EMBEDDINGS="${SIGLIP2_EMBEDDINGS:-$SIGLIP2_DIR/embeddings.npy}"
SIGLIP2_MODEL="${SIGLIP2_MODEL:-google/siglip2-base-patch16-224}"
SIGLIP2_ARGS=()
if [ -f "$SIGLIP2_EMBEDDINGS" ]; then
    echo "[+] SigLIP-2 artifacts detected ($SIGLIP2_EMBEDDINGS)."
    SIGLIP2_ARGS=(
        --siglip2-embeddings "$SIGLIP2_EMBEDDINGS"
        --siglip2-model-name "$SIGLIP2_MODEL"
    )
else
    echo "[-] SigLIP-2 embeddings not found at $SIGLIP2_EMBEDDINGS (skip if not used)."
fi
